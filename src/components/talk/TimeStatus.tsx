...
const TimeStatus = () => {
    ...
    function updateTime() {
        let current = new Date().toLocaleString("en-US", { timeZone: "Asia/Calcutta" });
        // console.log(current);
    ...
    return (
        <>
            <p className="text-black/50 dark:text-white/50 text-sm mb-10">
                It's currently <span className="font-semibold text-black/60 dark:text-white/60">{time}</span> for me, so
                I'm probably{" "}
                <span className="font-semibold text-black/60 dark:text-white/60">{awake ? "awake" : "sleeping"}</span>.
                I'll get back to you soon.
                <span className="font-bold text-white/90 flex pt-1">Fun Fact: I reply faster on email</span>
            </p>
        </>
    );
};
